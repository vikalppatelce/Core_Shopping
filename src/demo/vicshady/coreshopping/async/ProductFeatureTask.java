...
public class ProductFeatureTask extends AsyncTask<Void, Void, String>
{
	  ...
	  public ProductFeatureTask(Context context, ImageLoader imageLoader, DisplayImageOptions options,ListView productListView, String urlCatId)
	  {
...
//				jp = f.createJsonParser(context.getAssets().open("kadajsondata.txt"));
				jp = f.createJsonParser(RestClient.connect(AppConstants.URLS.PRODUCT_FEATURE_URL+urlCatId));
			} catch (JsonParseException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} catch (IOException e) {
