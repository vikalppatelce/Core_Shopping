...
public class ProductSearchTask extends AsyncTask<Void, Void, String>
{
	  ...
	  public ProductSearchTask(Context context, ImageLoader imageLoader, DisplayImageOptions options,ListView productListView, String urlSearchText)
	  {
		...
		this.urlSearchText = urlSearchText;
	  }
...
//				jp = f.createJsonParser(context.getAssets().open("kadajsondata.txt"));
				jp = f.createJsonParser(RestClient.connect(AppConstants.URLS.PRODUCT_SEARCH_URL+urlSearchText));
			} catch (JsonParseException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} catch (IOException e) {
